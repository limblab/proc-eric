function plot_pca_lpf(joint_interested,full_dataset,time_start, time_stop, data_full, data_truncated, pca4, pca5, pca6, pca7, pca8, pca9, pca10,e4, e5, e6, e7, e8, e9, e10, f2d4, f2d5, f2d6, f2d7, f2d8, f2d9, f2d10, train_ind_4, train_ind_5, train_ind_6, train_ind_7, train_ind_8, train_ind_9, train_ind_10)%,frames_to_drop)
...
pcs = [4 5 6 7 8 9 10];
supertitle = strcat(joint_interested,' - SVD PCA vs 5 Hz LPF - Full Dataset');
...
if full_dataset
    interpolated_fnums_4 = get_OG_fnums(f2d4, train_ind_4, data_full);
    interpolated_fnums_5 = get_OG_fnums(f2d5, train_ind_5, data_full);
    interpolated_fnums_6 = get_OG_fnums(f2d6, train_ind_6, data_full);
    interpolated_fnums_7 = get_OG_fnums(f2d7, train_ind_7, data_full);
    interpolated_fnums_8 = get_OG_fnums(f2d8, train_ind_8, data_full);
    interpolated_fnums_9 = get_OG_fnums(f2d9, train_ind_9, data_full);
    interpolated_fnums_10 = get_OG_fnums(f2d10, train_ind_10, data_full);
else
...
if full_dataset
    plot_frames_4 = get_interp_frames2plot(train_ind_4, fnum_plot_4, data_full);
    plot_frames_5 = get_interp_frames2plot(train_ind_5, fnum_plot_5, data_full);
    plot_frames_6 = get_interp_frames2plot(train_ind_6, fnum_plot_6, data_full);
    plot_frames_7 = get_interp_frames2plot(train_ind_7, fnum_plot_7, data_full);
    plot_frames_8 = get_interp_frames2plot(train_ind_8, fnum_plot_8, data_full);
    plot_frames_9 = get_interp_frames2plot(train_ind_9, fnum_plot_9, data_full);
    plot_frames_10 = get_interp_frames2plot(train_ind_10, fnum_plot_10, data_full);
else
...
title('X pos')
legend('rawData',strcat('5Hz LPF - Error = ',string(mean(e_lpf))), strcat(string(pcs(1)),'PCs - Error = ',string(mean(e4(3,:)))),strcat(string(pcs(2)),'PCs - Error = ', string(mean(e5(3,:)))),strcat(string(pcs(3)),'PCs - Error = ', string(mean(e6(3,:)))),strcat(string(pcs(4)),'PCs - Error = ',string(mean(e7(3,:)))),strcat(string(pcs(5)),'PCs - Error = ',string(mean(e8(3,:)))), strcat(string(pcs(6)),'PCs - Error = ', string(mean(e9(3,:)))), strcat(string(pcs(7)),'PCs - Error = ', string(mean(e10(3,:)))))
xlabel('sec')
ylabel('mm')
...
%saveas(f,strcat('/Users/ercrg/LimbLab/analysis/12_3/svd/',joint_interested,' - SVD PC comparison_full.png'))